...
avatar | String | - | \- | N
chat-content-props | Object | - | Typescript: `ChatMessageContentProps` `interface ChatMessageContentProps extends ChatContentProps { thinking?: MessageThinking; }` `type MessageThinking = Pick<ChatThinkingProps, 'animation' \| 'collapsed' \| 'layout' \| 'maxHeight'>  ` `import { ChatContentProps } from '@ChatContent'`，[ChatThinking API Documents](./chat-thinking?tab=api)。[see more ts definition](https://github.com/Tencent/tdesign-miniprogram/blob/develop/packages/pro-components/chat/chat-message/type.ts) | N
chat-id | String | - | \- | N
