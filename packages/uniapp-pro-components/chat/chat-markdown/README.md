...
streaming | Object | - | 流式输出配置，控制光标显示。不支持微信小程序端 markdown 组件的自定义光标组件能力。TS 类型：`TdChatStreamingConfig` `interface TdChatStreamingConfig { hasNextChunk?: boolean; tail?: boolean \| { content?: string } }`。[详细类型定义](https://github.com/tencent/tdesign-miniprogram/blob/develop/packages/uniapp-pro-components/chat/chat-markdown/type.ts) | N
